...
Tabs are a tagged union (`kind`: `terminal` | `editor` | `preview` | `markdown` | `ai-diff` | `git-diff` | `git-history` | `git-commit-file` | `sftp`). Tabs are **never unmounted on switch** — hidden via `invisible pointer-events-none` so PTYs and dev servers keep streaming. Terminal tabs own a `paneTree` (`PaneNode` leaf/split in `terminal/lib/panes.ts`, max `MAX_PANES_PER_TAB` = 9 panes); PTY/SSH sessions are keyed by leaf id globally and reused by `ensureSession`, so moving a leaf node between tabs preserves its live connection with zero reconnects. Multi-pane tabs show a per-pane connection chip (`PaneConnectionChip.tsx`, identity + status via pure `describePaneConnection`, reactive through multi-subscriber `subscribeSshStatus`).
Tab bar DnD (`TabBar.tsx`, mousedown + document mousemove/mouseup, no HTML5 DnD): the `Tabs` root uses `activationMode="manual"` so pressing a tab never selects it mid-drag. Drops over the bar reorder via `moveTab`; drops over the active tab content graft the whole dragged tab via `moveTabToSplit` (pure placement in `graftNode`, zone math / `canGraftSplit` / `isWorkspaceTree` in `tabs/lib/splitDrop.ts`, highlight in `SplitDropOverlay.tsx` portaled into the workspace content container). The drop target is captured at drag start (`DragState.targetId`), never the live selection, and App freezes the visible content on it (`onTabDragPreview` → `visibleActiveId` into `WorkspaceSurface`) so the dragged tab's content never flashes. Content drops never reorder. Self drops, non-terminal or block-mode tabs, and over-cap grafts are rejected (invalid overlay tint, drop no-ops). A private source escalates the target tab to private. Mixed-connection tabs (per-leaf keys from `getLeafConnectionKey`) display as "Workspace" via `labelFor(t, { workspace })`, which also reverts automatically on detach; grafting stamps each moved leaf with its origin tab name (`stampOriginTitle`, never the "Workspace" state) and `extractLeafToTab` ("Return to tab") restores it as `customTitle` with the pane's live session config (`getLeafSessionConfig`) so name, icon and connection travel with the pane. The remainder tab adopts the same identity when every leftover leaf shares one origin (`soleOriginTitle`), instead of keeping the stale container fields.
...
- **sftp/** — dual-pane file manager. Reuses `connect_and_auth` from ssh.
   - **Backend** — `sftp_open`/`sftp_close`/`sftp_list_dir` + mutations (`sftp_mkdir`/`sftp_rename`/`sftp_remove`/`sftp_chmod`) + file I/O (`sftp_read_file`/`sftp_write_file`/`sftp_download_file`) + transfers (`sftp_upload`/`sftp_download`/`sftp_upload_recursive`/`sftp_download_recursive`/`sftp_cancel`). Local-to-local copy lives in `fs/copy.rs` (`fs_copy`/`fs_copy_recursive`/`fs_copy_cancel`). All transfers stream in 64KB chunks, report throttled progress (200ms) via `Channel`, and support cancellation (`CancellationToken` per transfer in `SftpState`, `AtomicBool` in `CopyState`). `fs_read_dir` returns `mode` (unix permissions). `fs_open`/`fs_reveal`/`fs_chmod` in `fs/mutate.rs`.
   - **Frontend** — `SftpView` manages `Connection[]` per side (left/right) with sub-tabs. `SftpConnection` wraps `useLocalDir`/`useRemoteDir`/`useSftpEdit` hooks. `SftpPane` owns inline editing (`InlineEditState`), context menu, dialogs (delete, permissions), loading spinner.
   - **Drag-and-drop transfers** (Milestone 5) — drag rows between panes; `useSftpDragDrop` + `SftpDragContext` own the gesture, hit-testing via `document.elementFromPoint` + `data-sftp-pane`/`data-sftp-side`/`data-row-*` attributes; `useTransferQueue` runs a 3-worker pool routing via `dispatch()` (local↔local, upload, download, recursive); `SftpTransferQueue` shows live progress with cancel/retry and optional auto-close (pref `sftpCloseOnComplete`).
   - **Conflicts** (Milestone 6) — backend keeps skip-by-default and rejects existing destinations unless `overwrite`; frontend detects `destination exists`, marks transfer `conflict`, opens `SftpConflictDialog`, and supports overwrite, skip, rename, plus apply-to-all for the batch. Pure helpers live in `lib/transferConflicts.ts` (tested).
   - **Remote file editing** (Milestone 7) — downloads to temp, opens in editor, listens for `fs:file-written`, then enqueues visible upload via `useTransferQueue.enqueueRemoteEditUpload` so progress, failures, and retry appear in the transfer queue. Bookmarks per connection via `useSftpBookmarks(connKey)` persisted in `terax-sftp-bookmarks.json`. Pure routing helpers in `lib/transferRouting.ts` (tested).
- **editor/** — CodeMirror 6 stack. `extensions.ts` configures language modes, vim mode, and prebuilt themes.
- **ai/** — BYOK providers via `@ai-sdk/*` (`openai`, `anthropic`, `google`, `xai`, `cerebras`, `groq`, `deepseek`/`mistral` via `openai-compatible`, `openrouter`). Agent loop in `lib/agent.ts` (`runAgentStream` → `streamText` with `stopWhen: stepCountIs(MAX_AGENT_STEPS)`). Sessions persisted via `tauri-plugin-store` at `terax-ai-sessions.json`. Tools in `tools/tools.ts` — mutating tools gate on `needsApproval: () => ctx.getPermissionMode() !== "autonomous"` (permission-mode driven, not a static flag). Security deny-list in `lib/security.ts` applies to both read and write paths.
  - **Custom endpoints** (`CustomEndpoint` in `config.ts`: `baseURL`, `modelIds[]`, `contextLimit`, `headers?`, `api?: "auto" | "chat" | "responses" | "messages"`). `auto` (default) infers the wire protocol from the URL suffix (`…/chat/completions`, `…/responses`, `…/messages`); `normalizeCompatBaseURL` strips the suffix to the `…/v1` SDK prefix, so pasting a provider table's full REST URL works as-is. Dispatch in `buildLanguageModel`: `chat` → `createOpenAICompatible`, `responses` → `createOpenAI` (Responses API is its default) with `providerOptions.openai.store: false` (stateless steps; translating gateways like OpenCode Go reject stored-state tool continuations with "No function call found for function call output"), `messages` → `createAnthropic` with `authToken` Bearer plus `x-api-key` fallback. Every custom request sends `User-Agent: Terax/<package version>` (derived from `package.json`, never hardcoded), per-endpoint custom headers, and `x-opencode-session` = chat session id (stable per conversation; an explicit custom header wins). Session id flows `chatRuntime.makeChat` → `transport.createContextAwareTransport(getSessionId)` → `runAgentStream` → `buildConfiguredLanguageModel`. Model cache key includes api + headers + session id. Pure helpers (`inferCompatApi`, `resolveCompatApi`, `normalizeCompatBaseURL`, `resolveEffectiveProvider`, `mergeProviderOptions`) live in `config.ts` and are covered by `config.test.ts`. Stored-shape migrations in `modules/settings/store.ts` (`migrateEndpointModelIds`). Settings UI in `settings/sections/ModelsSection.tsx` (`CustomEndpointCard` with API selector Auto/Chat/Responses/Messages plus `HeadersField` key/value editor; for OpenCode Go create one endpoint per API sharing base `…/zen/go/v1`). **AI Sidebar** (`components/AiSidebar.tsx`) is the sole AI interface — lazy-loaded, resizable (min 360px, drag handle on left edge), per-tab open state managed in `App.tsx` via `useRef<Map<number, boolean>>`. Scope per tab: `terminal:<leafId>` (per active pane) or `workspace:<tabId>`. Each sidebar is fully independent: session, model/endpoint, permission mode, and active agent are all scoped independently via `selectedModelByScope`, `permissionModeByScope` in `chatStore`, and `activeIdByScope` in `agentsStore`. The transport in `chatRuntime.ts` resolves all values from the session's scope key at call time. The old mini-window, bottom input bar, and selection-ask-AI have been removed.
...
- **agents/** — agent notifications for the built-in Terax agent and terminal coding agents (Claude Code). Three passive detection sources: (1) `pty/agent_detect.rs` parses OSC 133/777 for local PTY shell integration, (2) `transcript_watcher.rs` polls `~/.claude/projects/**/*.jsonl` for local sessions and emits `terax:claude-transcript` with kind/context/projectDir, (3) `lib/claudeCliTracker.ts` tracks SSH PTY bytes for visual markers (`⏺`, spinner, permission keywords) with context extraction. All three route through `AgentNotificationsBridge.tsx` which dispatches OS notifications (via `lib/notify.ts`) and in-app toasts (via `AgentToast.tsx`), with descriptive context in the notification body. Shared store in `store/agentStore.ts`. OpenCode (local + SSH) notifies via a shipped plugin instead of sniffing: `modules/opencode_notify.rs` embeds `opencode_notify_plugin.js` (`session.idle` → finished, `session.error` → error, `permission.asked` → attention; never throws, pure fs append to `~/.terax-opencode-events.jsonl`), installs it locally (`opencode_notify_install_local`, version-marked, idempotent), tails the local file in a poll thread (`terax:opencode-signal`), and streams remote files over `ssh_tail_start/stop` exec channels (`tail -n0 -F`, `terax:opencode-tail` chunks + `terax:opencode-tail-closed`). Frontend `agents/lib/opencodeNotify.ts` owns parsing/line-buffering/ensure (local install on bridge mount, per-SSH-session version check + install + tail in `AgentNotificationsBridge`, routed to panes by cwd locally or by ssh session remotely). Pref `opencodeNotifications` (Settings → General → Agents).
