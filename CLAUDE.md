...
- **sftp/** — dual-pane file manager. Reuses `connect_and_auth` from ssh. Backend: `sftp_open`/`sftp_close`/`sftp_list_dir` + mutations (`sftp_mkdir`/`sftp_rename`/`sftp_remove`/`sftp_chmod`) + file I/O (`sftp_read_file`/`sftp_write_file`/`sftp_download_file`). `fs_read_dir` returns `mode` (unix permissions). `fs_open`/`fs_reveal`/`fs_chmod` in `fs/mutate.rs`. Frontend: `SftpView` manages `Connection[]` per side (left/right) with sub-tabs. `SftpConnection` wraps `useLocalDir`/`useRemoteDir`/`useSftpEdit` hooks. `SftpPane` owns inline editing (`InlineEditState`), context menu, dialogs (delete, permissions), loading spinner. Bookmarks per connection via `useSftpBookmarks(connKey)` persisted in `terax-sftp-bookmarks.json`. Remote file editing: download to temp, open in editor, upload on save via `fs:file-written` event listener.
