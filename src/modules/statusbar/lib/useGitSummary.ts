...
async function fetchSshSummary(sshId: number): Promise<GitQuickSummary | null> {
  ...
  const fetch = useCallback(async () => {
    try {
      ...
      if (isSSH && leafId !== null) {
        ...
        if (sshId !== null) {
          result = await fetchSshSummary(sshId);
        }
      } else if (!isSSH && cwd) {
        result = await fetchLocalSummary(cwd);
      }
      if (liveRef.current) setSummary(result);
    } catch {
      if (liveRef.current) setSummary(null);
    }
  }, [cwd, leafId, isSSH]);
